...
def check_security_hardening():
    ...
    with open(repofile('src/kotod'), 'rb') as f:
        magic = f.read(4)
        if not magic.startswith(b'\x7fELF'):
            return ret

    ret &= test_rpath_runpath('src/kotod')
    ret &= test_rpath_runpath('src/koto-cli')
    ret &= test_rpath_runpath('src/koto-gtest')
    ret &= test_rpath_runpath('src/koto-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/kotod')
    ret &= test_fortify_source('src/koto-cli')
    ret &= test_fortify_source('src/koto-gtest')
    ret &= test_fortify_source('src/koto-tx')
