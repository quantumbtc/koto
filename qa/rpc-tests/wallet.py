...
class WalletTest (BitcoinTestFramework):
    ...
    def run_test (self):
        ...
        # node0 should end up with 50 KOTO in block rewards plus fees, but
        # minus the 21 KOTO plus fees sent to node2
        ...
        # Send 10.0 KOTO from an utxo of value 100.0 KOTO but don't specify a change output, so then
        # the change of 90.0 KOTO becomes the fee, which is greater than estimated fee of 0.0021 KOTO.
        inputs = []
        outputs = {}
        ...
        outputs = {self.nodes[1].getnewaddress(): 96.998, self.nodes[0].getnewaddress(): 11.11}
        ...
        self.sync_all()
        assert_equal(self.nodes[2].getbalance(), Decimal('3802747.99799922')); #should not be changed because tx was not broadcasted
        assert_equal(self.nodes[2].getbalance("*"), Decimal('3802747.99799922')); #should not be changed because tx was not broadcasted
        ...
        self.sync_all()
        txObjNotBroadcasted = self.nodes[0].gettransaction(txIdNotBroadcasted)
        assert_equal(self.nodes[2].getbalance(), Decimal('3802749.99799922')); #should not be
        assert_equal(self.nodes[2].getbalance("*"), Decimal('3802749.99799922')); #should not be
        ...
        assert_equal(self.nodes[2].getbalance(), Decimal('3802751.99799922')); #should not be
        assert_equal(self.nodes[2].getbalance("*"), Decimal('3802751.99799922')); #should not be
        ...
        node2utxobalance = Decimal('3802761.998') - zsendmanynotevalue - zsendmanyfee
