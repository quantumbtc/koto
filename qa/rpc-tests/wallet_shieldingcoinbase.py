...
class WalletShieldingCoinbaseTest (BitcoinTestFramework):
    ...
    def run_test (self):
        ...
        error_result = wait_and_assert_operationid_status_result(self.nodes[0], myopid, "failed", ("Change 5.76542211 not allowed. "
        ...
        # This send will succeed.  We send two coinbase utxos totalling 200.0 less a default fee, with no change.
        shieldvalue = Decimal('194.0') - DEFAULT_FEE
        recipients = []
        ...
        assert_equal(Decimal(resp["private"]), Decimal('194.0') - DEFAULT_FEE)
        assert_equal(Decimal(resp["total"]), Decimal('3802691.0') - DEFAULT_FEE)
        ...
        recipients = []
        recipients.append({"address":myzaddr, "amount": Decimal('194.0') - DEFAULT_FEE})
        ...
        assert(Decimal(mempool[mytxid]['startingpriority']) >= Decimal('10000000000000'))
        ...
        assert_equal(Decimal(resp["private"]), Decimal('94.0') - 2*DEFAULT_FEE)
        assert_equal(Decimal(resp["total"]), Decimal('3802691.0') - 2*DEFAULT_FEE)
        check_value_pool(self.nodes[0], 'sprout', sproutvalue)
        ...
        # At this point in time, unspent notes all have a value of 100.0.
        recipients = []
        amount = Decimal('100.0') - DEFAULT_FEE - Decimal('0.00000001')    # this leaves change at 1 zatoshi less than dust threshold
        recipients.append({"address":self.nodes[0].getnewaddress(), "amount":amount })
        myopid = self.nodes[0].z_sendmany(mytaddr, recipients)
        wait_and_assert_operationid_status(self.nodes[0], myopid, "failed", "Insufficient transparent funds, have 100.00, need 0.00000053 more to avoid creating invalid change output 0.00000001 (dust threshold is 0.00000054)")
        ...
        try:
            self.nodes[0].sendtoaddress(self.nodes[2].getnewaddress(), 201)
        except JSONRPCException as e:
            errorString = e.error['message']
        ...
        try:
            self.nodes[0].z_sendmany(myzaddr, recipients, 1, Decimal('214160000.1'))
        except JSONRPCException as e:
            errorString = e.error['message']
        assert_equal("Amount out of range" in errorString, True)
