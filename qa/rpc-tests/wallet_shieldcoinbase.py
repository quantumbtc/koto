...
class WalletShieldCoinbaseTest (BitcoinTestFramework):
    ...
    def run_test (self):
        ...
        assert_equal(walletinfo['immature_balance'], (3920000+400)*0.97)
        ...
        self.sync_all()
        assert_equal(self.nodes[0].getbalance(), (3920000+400)*0.97)
        assert_equal(self.nodes[1].getbalance(), 100*0.97)
        assert_equal(self.nodes[2].getbalance(), 300*0.97)
        ...
        try:
            self.nodes[0].z_shieldcoinbase("*", myzaddr, Decimal('214160000.1'))
        except JSONRPCException as e:
            errorString = e.error['message']
        assert_equal("Amount out of range" in errorString, True)
        ...
        try:
            self.nodes[0].z_shieldcoinbase("*", myzaddr, 9999999)
        except JSONRPCException as e:
            errorString = e.error['message']
        ...
        assert_equal(self.nodes[0].getbalance(), 3920000*0.97)
        if self.addr_type == 'sprout':
            assert_equal(self.nodes[0].z_getbalance(do_not_shield_taddr), Decimal('3802400.0'))
            assert_equal(self.nodes[0].z_getbalance(myzaddr), Decimal('388.0') - DEFAULT_FEE)
        else:
            assert_equal(self.nodes[0].z_getbalance(do_not_shield_taddr), Decimal('3802400.0'))
            assert_equal(self.nodes[0].z_getbalance(myzaddr), Decimal('388.0') - DEFAULT_FEE)
        assert_equal(self.nodes[1].getbalance(), 200*0.97)
        assert_equal(self.nodes[2].getbalance(), 300*0.97)
        ...
        assert_equal(self.nodes[0].getbalance(), 3920000*0.97)
        if self.addr_type == 'sprout':
            assert_equal(self.nodes[0].z_getbalance(myzaddr), Decimal('679.0') - DEFAULT_FEE)
        else:
            assert_equal(self.nodes[0].z_getbalance(myzaddr), Decimal('679.0') - DEFAULT_FEE)
        assert_equal(self.nodes[1].getbalance(), 300*0.97)
