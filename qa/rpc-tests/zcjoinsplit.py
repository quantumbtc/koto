...
class JoinSplitTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        (total_in, inputs) = gather_inputs(self.nodes[0], 388)
        protect_tx = self.nodes[0].createrawtransaction(inputs, {})
        joinsplit_result = self.nodes[0].zcrawjoinsplit(protect_tx, {}, {zcaddress:387.99}, 387.99, 0)
        ...
        joinsplit_result = self.nodes[0].zcrawjoinsplit(joinsplit_tx, {receive_result["note"] : zcsecretkey}, {zcaddress: 387.98}, 0, 0.01)
