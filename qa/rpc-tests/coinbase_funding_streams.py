...
class CoinbaseFundingStreamsTest (BitcoinTestFramework):
    ...
    def run_test (self):
        ...
        assert_equal(walletinfo['immature_balance'], 3802497)
        assert_equal(walletinfo['balance'], 0)
        assert_equal(self.nodes[1].z_getbalance(miner_addr, 0), 194)
        assert_equal(self.nodes[1].z_getbalance(miner_addr), 194)
        ...
        assert_equal(walletinfo['immature_balance'], 3802497)
        assert_equal(walletinfo['balance'], 0)
        assert_equal(self.nodes[1].z_getbalance(miner_addr, 0), 582)
        assert_equal(self.nodes[1].z_getbalance(miner_addr), 582)

        # check that the node 0 private balance has been augmented by the
        # funding stream payments
        assert_equal(self.nodes[0].z_getbalance(fs_addr, 0), 0)
        assert_equal(self.nodes[0].z_getbalance(fs_addr), 0)
