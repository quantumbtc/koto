...
class MempoolLimit(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.sync_all()
        assert_equal(Decimal("3802400.00"), Decimal(self.nodes[1].z_gettotalbalance()['transparent']))
        assert_equal(Decimal("97.00"), Decimal(self.nodes[2].z_gettotalbalance()['transparent']))
        assert_equal(Decimal("97.00"), Decimal(self.nodes[3].z_gettotalbalance()['transparent']))
        ...
        opid1 = self.nodes[1].z_sendmany(get_coinbase_address(self.nodes[1]), [{"address": zaddr1, "amount": Decimal('3802400.0') - DEFAULT_FEE}])
        wait_and_assert_operationid_status(self.nodes[1], opid1)
        opid2 = self.nodes[2].z_sendmany(get_coinbase_address(self.nodes[2]), [{"address": zaddr2, "amount": Decimal('97.0') - DEFAULT_FEE}])
        ...
        opid3 = self.nodes[3].z_sendmany(get_coinbase_address(self.nodes[3]), [{"address": zaddr3, "amount": Decimal('97.0') - DEFAULT_FEE}])
        ...
        opid4 = self.nodes[0].z_sendmany(zaddr1, [{"address": zaddr4, "amount": Decimal('97.0') - 2*DEFAULT_FEE}])
        wait_and_assert_operationid_status(self.nodes[0], opid4)
        opid5 = self.nodes[0].z_sendmany(zaddr2, [{"address": zaddr4, "amount": Decimal('97.0') - 2*DEFAULT_FEE}])
