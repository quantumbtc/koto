...
std::pair<std::string, int64_t> GetWarnings(const std::string& strFor)
{
    ...
    std::pair<std::string, int64_t> gui;
    statusbar.second = rpc.second = gui.second = GetTime();
    int nPriority = 0;
    ...
    if (!CLIENT_VERSION_IS_RELEASE)
        statusbar.first = gui.first = _("This is a pre-release test build - use at your own risk - do not use for mining or merchant applications");
    ...
    if (strMiscWarning != "")
    {
        ...
        gui.first += (gui.first.empty() ? "" : uiAlertSeperator) + strMiscWarning;
        statusbar.second = gui.second = timestampWarning;
    }
    ...
    // Alerts
    {
        ...
        for (const std::pair<uint256, CAlert>& item : mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                gui.first += (gui.first.empty() ? "" : uiAlertSeperator) + alert.strStatusBar;
                statusbar.second = gui.second = GetTime();
