...
#if defined(NDEBUG)
# error "Koto cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Koto Signed Message:\n";
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (nActivationHeight != Consensus::NetworkUpgrade::NO_ACTIVATION_HEIGHT &&
        (pindex->nHeight >= nActivationHeight) && (pindex->nHeight <= chainparams.GetConsensus().GetLastFoundersRewardBlockHeight(pindex->nHeight))) {
        bool found = false;
    ...
    bool fCheckPOW)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetPoWHash_cached(), block.nBits, chainparams.GetConsensus()))
    ...
    const CChainParams& chainParams, CBlockIndex * const pindexPrev)
{
    ...
    // This retrospectively activates at block height 250 for mainnet and regtest,
    // and 851820 for testnet.
    ...
    const CChainParams& chainparams, CBlockIndex * const pindexPrev)
{
    ...
    // In Koto this has been enforced since launch, except that the genesis
    // block didn't include the height in the coinbase (see Koto protocol spec
    ...
    } else if (nActivationHeight != Consensus::NetworkUpgrade::NO_ACTIVATION_HEIGHT &&
               (nHeight >= nActivationHeight) && (nHeight <= consensusParams.GetLastFoundersRewardBlockHeight(nHeight))) {
#endif
    if (nActivationHeight != Consensus::NetworkUpgrade::NO_ACTIVATION_HEIGHT &&
        (nHeight >= nActivationHeight) && (nHeight <= consensusParams.GetLastFoundersRewardBlockHeight(nHeight))) {
