...
uint256 CTransaction::GetScriptSigHash() const
{
    ...
    if (!zcash_transaction_digests(
        ...
        nullptr))
    {
        throw std::ios_base::failure("CMutableTransaction::GetScriptSigHash: Invalid transaction format");
    }
    return hash;
}
...
std::string CTransaction::ToString() const
{
    ...
    if (!fOverwintered) {
        ...
        str += strprintf("CTransaction(txid=%s, hash=%s, ver=%d, fOverwintered=%d, nVersionGroupId=%08x, vin.size=%u, vout.size=%u, nLockTime=%u, nExpiryHeight=%u, valueBalanceSapling=%u, vSaplingSpend.size=%u, vSaplingOutput.size=%u",
            GetHash().ToString().substr(0,10),
            GetScriptSigHash().ToString().substr(0,10),
            nVersion,
            fOverwintered,
