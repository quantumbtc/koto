...
int printMiningStatus(bool mining)
{
    ...
    if (mining) {
        ...
        std::cout << _("To enable mining, add 'gen=1' to your koto.conf and restart.") << std::endl;
        lines += 2;
    }
    std::cout << std::endl;
            ...
            while (it != u->end()) {
                ...
                if (mapBlockIndex.count(hash) > 0 &&
                    ...
                    if (nActivationHeight != Consensus::NetworkUpgrade::NO_ACTIVATION_HEIGHT &&
                        (height >= nActivationHeight) && (height <= consensusParams.GetLastFoundersRewardBlockHeight(height))) {
...
void ThreadShowMetricsScreen()
{
    ...
    if (isScreen) {
        ...
        // Thank you text
        std::cout << strprintf(_("Thank you for running a %s kotod v%s node!"), WhichNetwork(), FormatVersion(CLIENT_VERSION)) << std::endl;
