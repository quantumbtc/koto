...
PACKAGE_VERSION=$($SRC_PATH/src/kotod --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/kotod $DEB_BIN
cp $SRC_PATH/src/koto-cli $DEB_BIN
cp $SRC_PATH/zcutil/fetch-params.sh $DEB_BIN/koto-fetch-params
# Copy docs
...
dpkg-shlibdeps $DEB_BIN/kotod $DEB_BIN/koto-cli
